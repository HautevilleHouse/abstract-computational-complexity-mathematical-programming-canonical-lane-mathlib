...
theorem source_import_dependency_count_checked : sourceImportDependencies.length = 0 := by rfl
theorem source_runtime_module_count_checked : sourceRuntimeModules.length = 0 := by rfl
theorem source_path_dependency_count_checked : sourcePathDependencies.length = 0 := by rfl
